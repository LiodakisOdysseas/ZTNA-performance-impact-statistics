...
for i in $(seq 1 50); do
  curl -s -w "@./utilities/metrics-format/ztna-metrics-format.txt" http://orion.openziti.eu:8080/v2/entities >> ./results/metrics/ztna-metrics.txt #ZTNA
  echo "Executed $i ZTNA transfers."
done

for i in $(seq 1 50); do
  curl -s -w "@./utilities/metrics-format/non-ztna-metrics-format.txt" http://orion.nonztna.openziti.eu:1027/v2/entities >> ./results/metrics/non-ztna-metrics.txt #non-ZTNA
